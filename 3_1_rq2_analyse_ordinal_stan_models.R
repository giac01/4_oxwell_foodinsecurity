...
dat_imputed_long = dat_imputed_long %>%
  mutate(
    secondary = (X1010_year %in% c("Y07","Y08","Y09","Y10","Y11","Y12","Y13"))
  )
...
emm_stan_fe[["swemwbs_ss"]] = analyse_stan_models(
  input_data        = dat_imputed_long %>% filter(secondary) , 
...
ggsave(filename = file.path("plots","3_1_stan_standardised_mean_differences_simple.pdf"), width = 8.6, height = 3.2)
ggsave(filename = file.path("plots","3_1_stan_standardised_mean_differences_simple.png"), width = 8.6, height = 3.2)
...
outcome_probabilities2 %>%
  ...
  geom_point(aes(shape =`Food Insecurity Level` ), size = 2, col = "black") +
  ...
  labs (
    ...
    y = "Model-Predicted Probability of Outcome Score",
    x = "Outcome Score"
    ) +
  theme(
...
ggsave(filename = file.path("plots","3_1_stan_marginal_effects_lineplot.pdf"), width = 9.5, height = 11)
ggsave(filename = file.path("plots","3_1_stan_marginal_effects_lineplot.png"), width = 9.5, height = 11, bg = "white")
...
ggsave(filename = file.path("plots","3_1_stan_marginal_effects_violinplot.png"), width = 9.5, height = 11)
ggsave(filename = file.path("plots","3_1_stan_marginal_effects_violinplot.pdf"), width = 9.5, height = 11)
...
combined_results_table %>%
  write.csv(file.path("results","3_1_stan_main_results_table.csv"))
...
# Gender Line Plot -------------------------------------------------------------
  ...
  ggsave(filename = file.path("plots","3_1_stan_rcads_gender_lineplot.pdf"), width = 9, height = 7)
  ggsave(filename = file.path("plots","3_1_stan_rcads_gender_lineplot.png"), width = 9, height = 7)
...
ggsave(file.path("plots","3_1_ORDINAL_posterior_predictive_checks.png"),  width = 7*1.4, height = 5*1.4)
