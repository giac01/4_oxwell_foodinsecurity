...
# stan_ordinal_results = readRDS(file.path("1_6_ordinal_results_allstudents.RDS"))
# stan_ordinal_results_old = readRDS(file.path("C:\\Users\\giaco\\OneDrive\\Work\\blakemore_postdoc\\Analyses\\4_oxwell\\backups\\PRE-REVISION-ANALYSES\\food_insecurity_2023_local\\r_output_enc\\1_6_ordinal_results.RDS")) 
stan_ordinal_results = readRDS(file.path("1_6_ordinal_results_17102024.RDS"))

# Only keep secondary school data ? --------------------------------------------
...
custom_colors <- c("#E6550D", "#3182BD", "#31A354")
# custom_colors <- c("#0077BE", "#FF7F00", "#31A354")
...
custom_breaks <- list(
  "RCADS-11 Depression" = c(0, 5, 10, 15),
  "RCADS-11 Anxiety" = c(0, 2, 4, 6, 8, 10),
  "Adolescent Wellbeing SWEMWBS" = c(0, 2, 4, 6, 8, 10),
  "Positive Thoughts" = c(0, 1, 2, 3),
  "Loneliness" = c(0, 5, 10, 15, 20, 25),
  "Child Wellbeing SCWBS" = c(0, 2, 4, 6, 8)
)
...
outcome_probabilities2 %>%
  ...
  scale_x_continuous(minor_breaks = 1:100,breaks = c(seq(0,100,by=2))) +
  scale_fill_manual(values = custom_colors) +
  scale_color_manual(values = custom_colors) +
  geom_point(aes(shape =`Food Insecurity Level` ), size = 2.5, col = "black") +
  geom_line(size =.6, col = "black") +
  facet_wrap(~(outcome), scales = "free", ncol = 2) +
  labs (
    title = "Predicted probability of each outcome score at different levels of food insecurity",
    y = "Model Predicted Probability",
    x = "Outcome Score"
    ) +
  guides(
    color = guide_legend(reverse = TRUE,override.aes = list(size = 9)),
  ...
  theme(
    ...
    panel.background = element_rect(fill = NA), # This makes the panel background transparent +
    legend.position = c(0.93, 0.08),      # Positioning the legend
    legend.justification = c(1, 0),

    legend.title = element_text(size = 14),  # Increase legend title size
    legend.text = element_text(size = 14),   # Increase legend text size
    legend.key.size = unit(1.75, "cm"),       # Increase size of legend keys
    legend.key.width = unit(1.75, "cm"),      # Adjust width of legend keys
    legend.spacing.x = unit(0.2, "cm"),      # Adjust horizontal spacing in legend
  ...
  ) # Adjusting the justification

ggsave(filename = file.path("plots","3_2_stan_marginal_effects_lineplot.pdf"), width = 9.5, height = 11)
ggsave(filename = file.path("plots","3_2_stan_marginal_effects_lineplot.png"), width = 9.5, height = 11, bg = "white")

## Manuscript Violin Plot ------------------------------------------------------

# Quick sanity check - numbers match up with other ones! 
# outcome_probabilities2 %>%
#   group_by(outcome, fi_level) %>%
#   mutate(yprob = (y*probability)) %>%
#   # summarise(sum(y*probability))
#   summarise(mean_outcome = sum(yprob)) %>%
#   print(n = 1000)

mean_results = main_results %>%
  filter(effect %in% paste0("mean",1:3)) %>%
  mutate(outcome = outcomes_labels[match(.$outcome, outcomes)]) %>%
  mutate(`Food Insecurity Level` = case_match(effect,
    "mean1" ~ "High",
    "mean2" ~ "Medium",
    "mean3" ~ "Low"
  )) %>%
...
outcome_probabilities2 %>%
  ...
  mutate(
    y_center = mean(y),
    prob_scaled = probability / max(probability),
    x_left = - prob_scaled / 2,
    x_right =  + prob_scaled / 2
  ) %>%
  ungroup() %>%
  filter(outcome!="Cognition") %>%
  # filter(outcome %in% outcomes_labels[1:4]) %>%
  # mutate(`Food Insecurity Level` = factor(`Food Insecurity Level`, 
  #                                         levels = c("Low","Medium","High"),
  #                                         ordered = TRUE)
  #        ) %>%
  ggplot(aes(y = y, group = `Food Insecurity Level`, fill = `Food Insecurity Level`)) +
  geom_ribbon(aes(xmin = 0 , xmax = probability), 
              color = "black", alpha = 0.8) +
  # geom_point(data = mean_results, aes(x = 0 , y = y)) +
  # geom_errorbar(data = mean_results, aes(x = 0 , ymin = ymin, ymax = ymax), width = .1) +
  facet_wrap(~ factor(outcome,levels=outcomes_labels), scales = "free") + 
                                # + factor(`Food Insecurity Level`, 
                                # levels = c("Low","Medium","High"),
                                # ordered = TRUE), scales = "free", ncol = 3) +
  scale_fill_manual(values = custom_colors) +
  scale_color_manual(values = custom_colors) +  
  ...
  theme(
    legend.position = c(1, 1),      # Positioning the legend
    legend.justification = c(1, 1),
    title = element_text(size = 12),
    strip.text = element_text(size = 12)
    )  # Adjusting the justification 

ggsave(filename = file.path("plots","3_2_stan_marginal_effects_violinplot.png"), width = 9.5, height = 11)
ggsave(filename = file.path("plots","3_2_stan_marginal_effects_violinplot.pdf"), width = 9.5, height = 11)
...
combined_results_table %>%
  write.csv(file.path("results","3_2_stan_main_results_table_justsecondaryschool.csv"))
